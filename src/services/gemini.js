...
// Rate limiting: RPM 15, RPD 500, TPM 250K
// ============================================================

import { state } from '../state.js';
import { sleep, showToast } from '../utils/helpers.js';
import { waitForRateLimit, recordRequest, estimateTokens, updateUsageUI } from '../utils/rate-limiter.js';

// Track if server key is exhausted this session
let serverKeyExhausted = false;
...
async function callGeminiProxy(prompt) {
  ...
  await waitForRateLimit(promptTokens);
  updateUsageUI('🔄 Mengirim ke server...');
  ...
  const responseTokens = estimateTokens(data.text);
  recordRequest(promptTokens, responseTokens);
  updateUsageUI('✅ Selesai');
...
async function callGeminiDirect(prompt, maxRetries = 2) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      // Enforce rate limits
      await waitForRateLimit(promptTokens);
      updateUsageUI('🔄 Mengirim request...');
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          updateUsageUI(`⏳ Rate limited, tunggu...`);
...
async function callGemini(prompt) {
  ...
  // If server key was previously exhausted, show guide
  ...
  try {
    ...
    if (serverKeyExhausted) {
      showApiKeyGuide();
      throw new Error('Kuota server habis. Masukkan API key Anda sendiri (gratis) di ⚙️ Settings.');
    }
...
/**
 * Show API key guide notification + modal
 */
function showApiKeyGuide() {
  ...
  showToast(
    '🔑 Kuota server habis! Dapatkan API key gratis di aistudio.google.com/apikey lalu masukkan di ⚙️ Settings.',
    'warning',
    15000
  );
      ...
      body.innerHTML = `
        <div style="line-height: 1.8; font-size: 0.9rem;">
          ...
          <ol style="padding-left: 20px;">
            <li style="margin-bottom: 8px;">Buka <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" style="color: #8b5cf6; font-weight: 600;">aistudio.google.com/apikey</a></li>
            ...
            <li style="margin-bottom: 8px;">Kembali ke sini → klik <strong>⚙️</strong> di header → paste key</li>
          </ol>
          <div style="background: rgba(139,92,246,0.1); border: 1px solid rgba(139,92,246,0.3); border-radius: 8px; padding: 12px; margin-top: 12px;">
            <strong>💡 Info:</strong> API key Google AI Studio gratis hingga 1500 request/hari. Tidak perlu kartu kredit.
          </div>
        </div>
      `;
      footer.innerHTML = `
        <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" class="btn btn-primary">🔗 Buka Google AI Studio</a>
...
export async function generateText(prompt) {
  ...
  if (provider === 'qwen') return callQwen(prompt);
...
export async function testConnection(provider) {
  ...
  try {
    if (provider === 'qwen') return await callQwen(testPrompt, 0);
    const userKey = state.get('settings.geminiKey')?.trim();
    if (userKey) return await callGeminiDirect(testPrompt, 0);
  ...
  try {
    ...
    if (jsonMatch) {
      try {
        ...
        throw new Error('Gagal parsing JSON. Coba generate ulang.');
      }
    }
